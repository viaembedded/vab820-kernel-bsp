...
#include <linux/ipu.h>
#include <linux/devices-common.h>
...
static int vdi_enc_setup(cam_data *cam)
{
	...
	ipu_csi_get_window_size(cam->ipu, &enc.mem_prp_vf_mem.in_width,
        ...
        pr_err("enc.mem_prp_vf_mem.out_width is %d, enc.mem_prp_vf_mem.out_height is %d,cam->v2f.fmt.pix.width is %d, cam->v2f.fmt.pix.height is %d,enc.mem_prp_vf_mem.in_width is %d,enc.mem_prp_vf_mem.in_height is %d\n",enc.mem_prp_vf_mem.out_width,enc.mem_prp_vf_mem.out_height, cam->v2f.fmt.pix.width, cam->v2f.fmt.pix.height,enc.mem_prp_vf_mem.in_width,enc.mem_prp_vf_mem.in_height);
	enc.mem_prp_vf_mem.out_width = cam->v2f.fmt.pix.width;
	enc.mem_prp_vf_mem.out_height = cam->v2f.fmt.pix.height;
//        enc.mem_prp_vf_mem.out_width = 1920;
//        enc.mem_prp_vf_mem.out_height = 1080;
	...
	err =
		ipu_init_channel_buffer(cam->ipu, MEM_VDI_PRP_VF_MEM, IPU_INPUT_BUFFER,
				...
				bytes_per_pixel(enc.mem_prp_vf_mem.
					out_pixel_fmt),
				IPU_ROTATE_NONE,
                cam->vdi_enc_bufs[NEXT_INDEX],0, 0,
				cam->offset.u_offset,
				cam->offset.v_offset);
	if (err != 0) {
...
static int vdi_enc_enabling_tasks(void *private)
{
	...
	CAMERA_TRACE("IPU:In vdi_enc_enabling_tasks\n");
        pr_err("IPU:In vdi_enc_enabling_tasks\n");
