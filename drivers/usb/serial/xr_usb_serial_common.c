...
//hobart
/*extern int xr_usb_serial_set_reg(struct xr_usb_serial *xr_usb_serial,int regnum, int value); 
bool SetHalfDuplex_xr21V141x(struct xr_usb_serial *xr_usb_serial, int nIndex)
{
	int nArgs[3] = {0};

	nArgs[0] = UART_0_REG_BLOCK + nIndex;
	nArgs[1] = UART_FLOW;

		printk("uart ioctl hobart!!");
	if(xr_usb_serial_set_reg(xr_usb_serial, XR_USB_SERIAL_GET_REG, &nArgs) < 0)
	
		printk("uart ioctl error1!!");
		return false;
	}
...
//	LOGI("uart flow: %02X", nArgs[2]);
	...
	if(xr_usb_serial_set_reg(xr_usb_serial, XR_USB_SERIAL_SET_REG, &nArgs) < 0)
	{
		printk("uart ioctl error2!!");
		return false;
	}
...
#endif
	// RS 485 half-dulpx test hobart 20150731
//printk("xr_usb_serial->dev is %d ",xr_usb_serial->major);
//SetHalfDuplex_xr21V141x(xr_usb_serial, minor);	
...
static int __init xr_usb_serial_init(void)
{
	...
	xr_usb_serial_tty_driver->name = "ttyUSB",
